...
from ots.utils import add_to_meters_in_dict, log_meters, print_meters, time_since, get_trainable_parameters, checkpoint_model, init_log
...
from ots.structures.bounding_box import nms, cat_boxlist
...
def mine_hard_patches(dataloader, net, cfg, criterion):
    # Get logger instance
    logger = logging.getLogger("OTS.mining_hard_patches")
...
def trainval_loop(dataloader_train, net, cfg, criterion, optimizer, dataloaders_eval=[]):
    ...
    logger = logging.getLogger("OTS.train")
    t_start = time.time()
