...
from ots.engine.objective import OTSObjective
from ots.utils import count_model_parameters, masked_select_or_fill_constant
from ots.structures.feature_map import FeatureMapSize
from .head import build_OTS_head_creator


def build_OTS_from_config(cfg):
    logger = logging.getLogger("OTS")
    logger.info("Building the OTS model")

    img_normalization = {"mean": cfg.model.normalization_mean, "std": cfg.model.normalization_std}
    net = OTSModel(logger=logger,
                   is_cuda=cfg.is_cuda,
    ...
    box_coder = OTSBoxCoder(positive_iou_threshold=cfg.train.objective.positive_iou_threshold,
                            ...
                            output_box_grid_generator=net.OTS_head_creator.box_grid_generator_image_level,
                            function_get_feature_map_size=net.get_feature_map_size,
                            do_nms_across_classes=cfg.eval.nms_across_classes)

    criterion = OTSObjective(class_loss=cfg.train.objective.class_objective,
    ...
    logger.info(f"OTS has {num_param_groups} blocks of {num_params} parameters (before freezing)")
    ...
    num_params, num_param_groups = count_model_parameters(net)
    logger.info(f"OTS has {num_param_groups} blocks of {num_params} trainable parameters")
...
class OTSModel(nn.Module):
    ...
    def __init__(self, logger,
        ...
        self.OTS_head_creator = build_OTS_head_creator(self.simplify_affine, is_cuda, self.use_inverse_geom_model,
    ...
    def freeze_transform_params(self):
        self.OTS_head_creator.aligner.parameter_regressor.eval()
        for param in self.OTS_head_creator.aligner.parameter_regressor.parameters():
    ...
    def forward(self, images=None, class_images=None,
        ...
        with torch.set_grad_enabled(train_mode and fine_tune_features):
            ...
            if class_head is None:
                ...
                class_head = self.OTS_head_creator.create_OTS_head(class_feature_maps)
    ...
    def init_model_from_file(self, path, init_affine_transform_path=""):
        ...
        if init_affine_transform_path:
            try:
                ...
                assert hasattr(self, "OTS_head_creator") and hasattr(self.OTS_head_creator, "aligner") and hasattr(self.OTS_head_creator.aligner, "parameter_regressor"), "Need to have the affine regressor part to inialize it"
                init_from_weakalign_model(model_data["state_dict"], None,
                                          affine_regressor=self.OTS_head_creator.aligner.parameter_regressor)
