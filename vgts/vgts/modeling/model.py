...
from .box_coder import VGTSBoxCoder, BoxGridGenerator
from vgts.engine.objective import VGTSObjective
from vgts.utils import count_model_parameters, masked_select_or_fill_constant
from vgts.structures.feature_map import FeatureMapSize
from .head import build_VGTS_head_creator


def build_VGTS_from_config(cfg):
    logger = logging.getLogger("VGTS")
    logger.info("Building the VGTS model")

    img_normalization = {"mean": cfg.model.normalization_mean, "std": cfg.model.normalization_std}
    net = VGTSModel(logger=logger,
                   is_cuda=cfg.is_cuda,
    ...
    box_coder = VGTSBoxCoder(positive_iou_threshold=cfg.train.objective.positive_iou_threshold,
                            ...
                            output_box_grid_generator=net.VGTS_head_creator.box_grid_generator_image_level,
                            function_get_feature_map_size=net.get_feature_map_size,
                            do_nms_across_classes=cfg.eval.nms_across_classes)

    criterion = VGTSObjective(class_loss=cfg.train.objective.class_objective,
    ...
    logger.info(f"VGTS has {num_param_groups} blocks of {num_params} parameters (before freezing)")
    ...
    if num_frozen_extractor_blocks > 0:
        logger.info(f"Freezing {num_frozen_extractor_blocks} of {net.get_num_blocks_in_feature_extractor()} blocks of the feature extractor network")
        net.freeze_extractor_blocks(num_blocks=num_frozen_extractor_blocks)

    num_params, num_param_groups = count_model_parameters(net)
    logger.info(f"VGTS has {num_param_groups} blocks of {num_params} trainable parameters")
...
def get_feature_map_size_for_network(img_size, net, is_cuda=False):
    dummy_image = torch.zeros(1, 3, img_size.h, img_size.w) 
    if is_cuda:
...
class VGTSModel(nn.Module):
    ...
    def __init__(self, logger,
        ...
        self.VGTS_head_creator = build_VGTS_head_creator(self.simplify_affine, is_cuda, self.use_inverse_geom_model,
    ...
    def freeze_transform_params(self):
        self.VGTS_head_creator.aligner.parameter_regressor.eval()
        for param in self.VGTS_head_creator.aligner.parameter_regressor.parameters():
    ...
    def forward(self, images=None, class_images=None,
        ...
        with torch.set_grad_enabled(train_mode and fine_tune_features):
            ...
            if class_head is None:
                ...
                class_head = self.VGTS_head_creator.create_VGTS_head(class_feature_maps)
    ...
    def init_model_from_file(self, path, init_affine_transform_path=""):
        ...
        if init_affine_transform_path:
            try:
                ...
                assert hasattr(self, "VGTS_head_creator") and hasattr(self.VGTS_head_creator, "aligner") and hasattr(self.VGTS_head_creator.aligner, "parameter_regressor"), "Need to have the affine regressor part to inialize it"
                init_from_weakalign_model(model_data["state_dict"], None,
                                          affine_regressor=self.VGTS_head_creator.aligner.parameter_regressor)
