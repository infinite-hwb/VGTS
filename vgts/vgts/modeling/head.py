...
from vgts.structures.bounding_box import BoxList, cat_boxlist


def build_VGTS_head_creator(do_simple_affine, is_cuda, use_inverse_geom_model, feature_map_stride, feature_map_receptive_field):
    aligner = VGTSAlignment(do_simple_affine, is_cuda, use_inverse_geom_model)
    head_creator = VGTSHeadCreator(aligner, feature_map_stride, feature_map_receptive_field)
    return head_creator
...
class VGTSHeadCreator(nn.Module):
    ...
    def get_rec_field_and_stride_after_concat_nets(receptive_field_netA, stride_netA, receptive_field_netB, stride_netB):
        if isinstance(receptive_field_netA, FeatureMapSize):
            ...
            rec_field_w, stride_w = VGTSHeadCreator.get_rec_field_and_stride_after_concat_nets(receptive_field_netA.w, stride_netA.w, receptive_field_netB.w, stride_netB.w)
            rec_field_h, stride_h = VGTSHeadCreator.get_rec_field_and_stride_after_concat_nets(receptive_field_netA.h, stride_netA.h, receptive_field_netB.h, stride_netB.h)
    ...
    def create_VGTS_head(self, class_feature_maps):
        ...
        return VGTSHead(class_feature_maps_ref_size, self.aligner, self.box_grid_generator_image_level, self.box_grid_generator_feature_map_level)


class VGTSHead(nn.Module):
    def __init__(self, class_feature_maps, aligner,
                       box_grid_generator_image_level,
                       box_grid_generator_feature_map_level,
                       pool_border_width=2):
        super(VGTSHead, self).__init__()
    ...
    def forward(self, feature_maps):
        ...
        output_localization = VGTSBoxCoder.build_loc_targets(class_boxes, default_boxes_with_image_batches)
