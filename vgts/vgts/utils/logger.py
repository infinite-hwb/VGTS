...
def log_meters(log, t_start, i_iter, log_path,
    ...
    logger = logging.getLogger("VGTS.logger")
...
def checkpoint_model(net, optimizer, log_path, is_cuda, model_name=None, i_iter=None, extra_fields=None):
    logger = logging.getLogger("VGTS.checkpoint")
    net.cpu()
    try:
        if not os.path.isdir(log_path):
            os.makedirs(log_path)
...
def extract_map_value_from_VGTS_log(result_file, eval_dataset):
...
def extract_value_from_VGTS_binary_log(data_file, value, reduce):
    try:
