...
from vgts.loaddata.voc_eval import do_voc_evaluation
from vgts.utils import add_to_meters_in_dict, print_meters, time_since, time_for_printing
from vgts.modeling.model import get_feature_map_size_for_network
...
def evaluate(dataloader, net, cfg, criterion=None, print_per_class_results=False):
    # Get logger
    logger = logging.getLogger("VGTS.evaluate")
    ...
    for data in iterator:
        ...
        boxes_one_image = dataloader.box_coder.decode_pyramid(image_loc_scores_pyramid, image_class_scores_pyramid,
...
def make_iterator_extract_scores_from_images_batched(dataloader, net, logger, image_batch_size, is_cuda,
    ...
    for i in range(0, num_classes, class_batch_size):
        ...
        # Extract feature maps from batch images and create corresponding vgts head
        for b_im in batch_class_images:
            class_feature_maps = net.net_label_features([b_im])
            class_conv_layer = net.VGTS_head_creator.create_VGTS_head(class_feature_maps)
